...
import { Search, Bell, ChevronDown, Shield, Brain, X, User, FileText, Sparkles } from "lucide-react";
...
export default function AdminTopBar() {
  ...
  useEffect(() => {
    ...
    // Mock results
    setSearchResults([
      { id: "1", type: "user", title: "John Doe", subtitle: "Manager • Active", href: "/admin/users" },
      { id: "2", type: "log", title: "System Update", subtitle: "Completed successfully", href: "/admin/settings" },
    ]);
  }, [searchQuery]);
  ...
  return (
    <header className="fixed top-0 left-24 right-0 h-16 bg-white border-b border-neutral-200 flex items-center justify-between px-6 z-40">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 px-3 py-1.5 bg-primary-blue/10 text-primary-blue rounded-full">
          <Shield size={16} />
      ...
      <div className="flex-1 max-w-xl mx-8 relative" ref={searchRef}>
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search system logs, users, actions..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            onFocus={() => setIsSearchFocused(true)}
            className="w-full pl-10 pr-4 py-2 bg-neutral-50 border border-neutral-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-primary-blue/20 focus:border-primary-blue transition-all"
          />
          {searchQuery && (
            <button
            ...
            {searchResults.map((result) => (
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-neutral-100 flex items-center justify-center">
                  {result.type === "user" ? <User size={16} /> : <FileText size={16} />}
                </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="px-4 py-2 bg-primary-blue/10 hover:bg-primary-blue/20 text-primary-blue font-semibold rounded-full transition-all flex items-center gap-2 border border-primary-blue/20"
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-status-success/10 rounded-full">
          ...
          <span className="text-xs font-medium text-status-success">AI Online</span>
        </div>
          ...
          {showNotifications && (
            <div className="absolute top-full right-0 mt-2 w-80 bg-white border border-neutral-200 rounded-xl shadow-lg overflow-hidden z-50">
              <div className="px-4 py-3 border-b border-neutral-100 flex items-center justify-between">
                ...
                <button className="text-xs text-primary-blue font-medium">Mark all read</button>
              </div>
              <div className="max-h-80 overflow-y-auto p-2 space-y-1">
                <div className="p-3 hover:bg-neutral-50 rounded-lg cursor-pointer">
                  <p className="text-sm font-medium text-neutral-900">System Alert</p>
                  <p className="text-xs text-neutral-500">Database backup completed</p>
                </div>
                <div className="p-3 hover:bg-neutral-50 rounded-lg cursor-pointer">
                  <p className="text-sm font-medium text-neutral-900">New User</p>
                  <p className="text-xs text-neutral-500">Registration request pending</p>
                </div>
              </div>
            </div>
          )}
        ...
        <button className="flex items-center gap-3 px-3 py-1.5 hover:bg-neutral-50 rounded-full transition-colors">
          <div className="w-8 h-8 rounded-full bg-primary-blue flex items-center justify-center text-white font-semibold text-sm">
