...
export default function PharmacistTopBar() {
  ...
  useEffect(() => {
    ...
    // Mock results
    setSearchResults([
      { id: "1", type: "medicine", title: "Amoxicillin 500mg", subtitle: "Rack A12 • Stock: 45", href: "/pharmacist/inventory" },
      { id: "2", type: "prescription", title: "Rx #9988", subtitle: "Dr. Smith • Pending", href: "/pharmacist/dashboard" },
    ]);
  }, [searchQuery]);
  ...
  return (
    <header className="fixed top-0 left-24 right-0 h-16 bg-white border-b border-neutral-200 flex items-center justify-between px-6 z-40">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 px-3 py-1.5 bg-primary-green/10 text-primary-green rounded-full">
          <Pill size={16} />
          <span className="text-sm font-semibold">Pharmacist Portal</span>
        </div>
      ...
      <div className="flex-1 max-w-md mx-8 relative" ref={searchRef}>
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-neutral-50 border border-neutral-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-primary-green/20 focus:border-primary-green transition-all"
          />
          {searchQuery && (
            <button
            ...
            {searchResults.map((result) => (
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-neutral-100 flex items-center justify-center">
                  {result.type === "medicine" ? <Package size={16} /> : <FileText size={16} />}
                </div>
      ...
      <div className="flex items-center gap-4">
          ...
          {showNotifications && (
            <div className="absolute top-full right-0 mt-2 w-80 bg-white border border-neutral-200 rounded-xl shadow-lg overflow-hidden z-50">
              <div className="px-4 py-3 border-b border-neutral-100 flex items-center justify-between">
                ...
                <button className="text-xs text-primary-green font-medium">Mark all read</button>
              </div>
              <div className="max-h-80 overflow-y-auto p-2 space-y-1">
                <div className="p-3 hover:bg-neutral-50 rounded-lg cursor-pointer">
                  <p className="text-sm font-medium text-neutral-900">New Prescription</p>
                  <p className="text-xs text-neutral-500">Received from Dr. Smith</p>
                </div>
                <div className="p-3 hover:bg-neutral-50 rounded-lg cursor-pointer">
                  <p className="text-sm font-medium text-neutral-900">Stock Alert</p>
                  <p className="text-xs text-neutral-500">Amoxicillin is running low</p>
                </div>
              </div>
            </div>
          )}
        ...
        <button className="flex items-center gap-3 px-3 py-1.5 hover:bg-neutral-50 rounded-full transition-colors">
          <div className="w-8 h-8 rounded-full bg-primary-green flex items-center justify-center text-white font-semibold text-sm">
            {user?.name?.charAt(0).toUpperCase() || "P"}
          </div>
          <div className="text-left">
            <div className="text-sm font-medium text-neutral-900">{user?.name || "Pharmacist"}</div>
            <div className="text-xs text-neutral-500">Pharmacist</div>
          </div>
